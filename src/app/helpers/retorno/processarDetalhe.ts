...
export default async (linha) => {
  ...
  const ocorrencia_descricao = await util.buscaDescricao(linha.substr(231, 10).trim());
  ...
  const dados = {
    ...
    data_prevista_pagamento: moment(linha.substr(94, 8), 'DD/MM/YYYY').format("DD/MM/YYYY"),
    tipo_moeda: linha.substr(102, 3),
    codigo_ISPB: linha.substr(105, 112),
    valor_previsto_pagamento: (parseFloat(linha.substr(120, 15).replace(/^0+/, '')) * 0.01).toFixed(2),
    nosso_numero: linha.substr(135, 15).trim(),
    data_efetivacao_pagamento: moment(linha.substr(155, 8), 'DD/MM/YYYY').format("DD/MM/YYYY"),
    valor_efetivo_pagamento: (parseFloat(linha.substr(163, 15).replace(/^0+/, '')) * 0.01).toFixed(2),
    ...
    descricao_ocorrencia: ocorrencia_descricao ? ocorrencia_descricao.mensagem : '',
    id_codigo_retorno_ocorrencia: ocorrencia_descricao ? ocorrencia_descricao.id : '',
    ocorrencia_status: ocorrencia_descricao ? ocorrencia_descricao.ocorrencia_status : '',
    ocorrencia_responsabilidade_adiante: ocorrencia_descricao ? ocorrencia_descricao.ocorrencia_responsabilidade_adiante : '',
    ted_id: buscaTedId ? buscaTedId : ''
  }
  return dados;
}