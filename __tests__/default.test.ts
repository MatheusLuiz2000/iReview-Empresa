...
describe('Apidoc', () => {
  ...
  it('Deve dar certo a criacao da solicitação de ted', async () => {
    axiosMock
      .onGet(new RegExp(`${process.env.CLIENTE_BASE}/*`))
      .reply(200, ClienteTest);
  ...
  it('Deve dar certo porém deve retornar dados nulo devido que não existe nenhuma solicitação de TED.', async () => {
    ...
    const response = await request(app).post('/teds/gerar/');
  ...
  it('Deve dar certo porém deve ser efetuada na mão devido que que o horario é menor que 13:00', async () => {
    ...
    expect(response.body.data.retorno[0].motivo).toBe(
      'Ted deve ser efetuada manualmente'
    );
    expect(response.status).toBe(200);
  });
  ...
  it('Deve dar certo e ler o arquivo corretamente', async () => {
    const response = await request(app)
      ...
      .send({
        link_s3:
          'https://adiantesa-dev.s3.amazonaws.com/uploads/retorno/deposito/2019/10/REMESSA16860_1809201946.RET.20190918141820410'
      });
    ...
    const { json_retorno } = buscaretornoTed.dataValues;
